...
namespace Dos.Common
{
    ...
    public class HttpHelper
    {
        ...
        public static Stream RequestStream(HttpParam param)
        {
            ...
            {
                foreach (var kv in param.Headers)
                {
                    if (kv.Key.ToLower() == "user-agent")
                    {
                        r.UserAgent = kv.Value;
                        continue;
                    }
                    else if(kv.Key.ToLower()== "content-type")
                    {
                        param.ContentType = kv.Value;
                        continue;
                    }
            ...
            if (param.PostParam != null)
            {
                ...
                {
                    if (param.ContentType.ToLower().Contains("json"))
                    {
                    ...
                    {
                        getParamSb.Clear();
                        param.PostParam.GetType().GetProperties().ToList().ForEach(d =>
                        {
                            getParamSb.AppendFormat("{0}={1}&", d.Name, d.GetValue(param.PostParam, null));
                        });
                        if (!string.IsNullOrWhiteSpace(getParamSb.ToString().TrimEnd('&')))
                        {
                            postParamString = string.Format("{0}?{1}", param.Url, getParamSb.ToString().TrimEnd('&'));
                        }
                    }
        ...
        /// <param name="postParam"></param>
        /// <returns></returns>
        public static string Put(string url, object postParam)
        {
            var param = new HttpParam
            {
                Url = url,
        ...
        /// <param name="postParam"></param>
        /// <returns></returns>
        public static T Put<T>(string url, object postParam)
        {
            var param = new HttpParam
            {
                Url = url,
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public static T Put<T>(HttpParam param)
        {
            param.Method = "PUT";
            var str = Put(param);
            return JSON.ToObject<T>(str);
