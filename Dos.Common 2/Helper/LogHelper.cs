...
namespace Dos.Common
{
    ...
    public class LogHelper
    {
        ...
        public static void Info(string content, string filePrefixName = null, string path = null)
        {
            Write(LogHelperType.info, content, filePrefixName, path);
        }
        ...
        private static void Write(LogHelperType logtype, string content, string filePrefixName = null, string path = null)
        {
            lock (Olock)
            {
                try
                {
                    ...
                    else if (logtype == LogHelperType.error)
                    {
                    ...
                    else if (logtype == LogHelperType.info)
                    {
                        var dosError = ConfigurationManager.AppSettings["Dos.LogHelper.Info"];
                        if (dosError != null && dosError != "1")
                        {
                            return;
                        }
        ...
        public static void Clean(string path = null)
        {
            ...
            try
            {
                FileInfo[] fs = new DirectoryInfo(path).GetFiles("*.txt", SearchOption.TopDirectoryOnly);
                DateTime before7day = DateTime.Now-TimeSpan.FromDays(7);
                foreach(var f in fs)
                {
                    if(f.CreationTime< before7day)
                    {
                        f.Delete();
                    }
                }
